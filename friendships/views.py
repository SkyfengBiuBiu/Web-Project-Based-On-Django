...
def request(request, user_id):
    ...
    if request.user.id is None:
        ...
        if request.method == "GET": #when request is GET, list 3 list: send list, receive list and notification list
            ...
            request_sending_list = FriendshipRequest.objects.filter(sender_id=user_id, status=FriendshipRequest.INITIAL)
            request_receive_list = FriendshipRequest.objects.filter(recipient_id=user_id, status=FriendshipRequest.INITIAL)
            ...
            request_decline_notification_list = FriendshipRequest.objects.filter(sender_id=user_id, status=FriendshipRequest.DECLINED)
            ...
            if request.POST.get('SendOrRecvOrFriendOrNotification') == 'send':
                ...
                FriendshipRequest.objects.filter(sender_id=sender_id, recipient_id=recipient_id, status=FriendshipRequest.INITIAL).delete()#'send' POST means delete send request
                ...
                if request.POST.get('status') == 'accept':
                    ...
                    FriendshipRequest.objects.filter(sender_id=sender_id, recipient_id=recipient_id, status=FriendshipRequest.INITIAL).update(status=FriendshipRequest.ACCEPTED, date=timestamp)#once accept, update status to 'accept'
                    ...
                    FriendshipRequest.objects.filter(sender_id=sender_id, recipient_id=recipient_id, status=FriendshipRequest.INITIAL).update(status=FriendshipRequest.DECLINED, date=timestamp)#once decline,update status to 'decline'and timing of decline
                ...
                FriendshipRequest.objects.filter(sender_id=user1_id, recipient_id=user2_id, status=FriendshipRequest.ACCEPTED).delete()#once forming friend from one user, reverse send request will be deleted
                ...
                sender_id = request.POST.get('sender_id')
                recipient_id = request.POST.get('recipient_id')
                FriendshipRequest.objects.filter(sender_id=sender_id, recipient_id=recipient_id, status=FriendshipRequest.DECLINED).delete()#detele notification
...
def profile(request, user_id, recipient_id):
    ...
    if request.user.id is None:
        ...
        if request.method == "GET":
            ...
            re_reqst_chk = FriendshipRequest.objects.filter(sender_id=sender_id, recipient_id=recipient_id, status=FriendshipRequest.INITIAL).count()
            ...
            if (re_reqst_chk == 0) and (friend_chk_p == 0) and (friend_chk_ == 0):#if send request have been sent or recipient is friend already, send request can not be sent
                ...
                FriendshipRequest.objects.create(sender_id=sender_id, recipient_id=recipient_id, date=timestamp, status=FriendshipRequest.INITIAL)
